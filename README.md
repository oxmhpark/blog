NEMORIUM.NET
============

> [옥수박](https://github.com/oxmhpark)의 개인 블로그.

# 적용 기술

| 항목 | 제품 | 링크 |
|---|---|---|
| 플랫폼 | 깃허브 페이지스 | <https://pages.github.com> |
| 블로그 | 지킬 | <https://jekyllrb.com> |
| 댓글 | giscus | <https://giscus.app> |
| 검색 | GPSE | <https://programmablesearchengine.google.com> |
| 글꼴 | Freesentation | <https://freesentation.blog> |

# 고정 페이지

관리 목적이 아니라면 추가할 일이 없을 것이다.

| 항목 | 소스 경로 |
|---|---|
| 첫 페이지 | [`/index.md`](https://github.com/oxmhpark/blog/tree/main/index.md) |
| 아카이브 | [`/_entry_pages/archives.md`](https://github.com/oxmhpark/blog/tree/main/_entry_pages/archives.md) |
| 즐겨찾기 | [`/_entry_pages/bookmarks.md`](https://github.com/oxmhpark/blog/tree/main/_entry_pages/bookmarks.md) |
| 소개 | [`/_entry_pages/about.md`](https://github.com/oxmhpark/blog/tree/main/_entry_pages/about.md) |
| RSS 피드 | [`/feed.xml`](https://github.com/oxmhpark/blog/tree/main/feed.xml) |
| 사이트맵 | [`/sitemap.xml`](https://github.com/oxmhpark/blog/tree/main/sitemap.xml) |

# 분류 방침

대분류를 제외한 나머지 범주는 복수설정이 가능하다.

| 범주 | 설정 | 템플릿 경로 |
|---|---|---|
| 대분류 | 행위의 종류<br>콜렉션(=폴더) 분리 | `대분류` 단락에서 기술 |
| 중분류 | 구체적인 게시 목적(=프로젝트)<br>`categories` 프론트매터 지정<br>개별 템플릿 작성 | [`/_archive_categories/`](https://github.com/oxmhpark/blog/tree/main/_archive_categories) |
| 소분류 | 구체적이고 반복인용 가능성이 있는 소재<br>`tags` 프론트매터 지정[^about-tags]<br>개별 템플릿 작성 | [`/_archive_tags/`](https://github.com/oxmhpark/blog/tree/main/_archive_tags) |
| 연도 | 게시물이 속하거나 취급하는 연도.<br>`years` 프론트매터 지정<br>개별 템플릿 작성 | [`/_archive_years/`](https://github.com/oxmhpark/blog/tree/main/_archive_years) |

[^about-tags]: 예를 들어, '인물'은 부적절하고 '시로 마사무네'는 적절하다.

## 대분류

| 콜렉션 이름 | 설정 | 폴더 경로 |
|---|---|---|
| 읽기 | 텍스트 요약(평가하지 않음) | [`/_entry_reads/`](https://github.com/oxmhpark/blog/tree/main/_entry_reads/) |
| 배우기 | 텍스트 평가·주제 연구 | [`/_entry_studies/`](https://github.com/oxmhpark/blog/tree/main/_entry_studies/) |
| 쓰기 | 사설·창작 | [`/_entry_writes/`](https://github.com/oxmhpark/blog/tree/main/_entry_writes/) |
| 놀기 |  | [`/_entry_plays/`](https://github.com/oxmhpark/blog/tree/main/_entry_plays/) |

# 운영 방침

* 이미지 업로드, 토막글 저장은 [블루스카이](https://bsky.app/profile/nemorium.net)를 이용한다.
* 텍스트 제목에는 겹낫표(『』)를, 챕터 제목에는 홑낫표(「」)를 쓴다.[^punc-title]
* 인물의 대사 등 직접 인용에는 겹따옴표(“”)를, 인물의 생각 등 간접 인용에는 홑따옴표(‘’)를 쓴다.[^punc-quote]
* 문서 구조를 단순하게 유지하고 주요 갱신이력을 남길 것.
* 소통에 2차 비용을 치르지 말 것.

[memo]: https://memo.nemorium.net/@oxmhpark