...
개인 블로그인 만큼 방문객의 검색 편의성보다는 관리 편의성을 중시하기로 한다. 윈도우즈 11의 특정 기능에 대해 다루는 게시물에 `운영체제` `윈도우즈` `윈도우즈 11` 소분류를 모두 지정하는 방식은 `터미널` `파워쉘` 같은 다른 소분류들도 같은 기준 아래 대량으로 추가되면서 오탈자·누락 가능성이 커진다.

# 운영 방침

* 이미지 업로드, 글감과 토막글 저장은 [메모장](https://memo.nemorium.net)을 이용한다.
* 논조에 변화가 있다면 별도의 게시물로 분리하고, 논거를 추가하는 정도라면 서두에 주요 갱신 이력을 남긴다.
